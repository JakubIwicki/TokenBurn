...
///     <see cref="EmbeddingException" />; no retry is performed here because
///     hybrid search degrades to keyword-only fusion on an embedding failure.
/// </summary>
    ...
    ILogger<TextEmbeddingsInferenceClient> logger) : IEmbeddingClient
{
    ...
    public async Task<IReadOnlyList<float>> EmbedAsync(IReadOnlyList<string> texts, CancellationToken cancellationToken)
    {
        ...
        // Hybrid search embeds a single query; TEI echoes one vector per input, so the
